...
function makeTask(name, displayName) {
    return {
        name,
        displayName,
        resolveSkill: vi.fn(),
...
describe('createDefaultCallbacks', () => {
    ...
    describe('onSkillComplete', () => {
        it('logs completion with duration and finding summary in log mode (Normal verbosity omits per-finding lines)', () => {
            ...
            expect(errorSpy).toHaveBeenCalledTimes(1);
            const completionMsg = errorSpy.mock.calls[0][0];
        ...
        it('shows suggested fix only at Debug verbosity in log mode', () => {
            ...
            const fixMsg = errorSpy.mock.calls[2][0];
...
describe('runSkillTasks', () => {
    ...
    function makeAbortedTasks() {
        const controller = new AbortController();
        controller.abort();
        const resolveSkill = vi.fn();
        const context = { eventType: 'pull_request', repository: { owner: 'o', name: 'n', fullName: 'o/n', defaultBranch: 'main' }, repoPath: '/tmp' };
        const tasks = [
