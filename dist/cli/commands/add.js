...
import { fetchRemote, discoverRemoteSkills, parseRemoteRef, } from '../../skills/remote.js';
...
const selectTheme = {
    ...
    style: {
        ...
        highlight: (text) => chalk.white(text),
        disabled: (text) => chalk.dim(text),
        description: (text) => chalk.white(text),
        keysHelpTip: (keys) => {
 ...
 */
function renderSkillList(skills, configuredSkills, reporter) {
 ...
 */
function renderRemoteSkillList(skills, configuredSkills, remote, reporter) {
 ...
 */
async function promptSkillSelection(skills, configuredSkills, reporter) {
 ...
 */
async function promptRemoteSkillSelection(skills, configuredSkills, reporter) {
    // Filter out already configured skills for selection
 ...
 */
function createDefaultSkillConfig(skillName) {
    return {
        name: skillName,
 ...
 */
function createRemoteSkillConfig(skillName, remote) {
    return {
        name: skillName,
        remote,
        triggers: DEFAULT_TRIGGERS,
 ...
 */
async function runAddRemote(options, reporter, configPath, configuredSkills) {
 ...
 * Run the add command.
 */
export async function runAdd(options, reporter) {
    const cwd = process.cwd();
    // 1. Check git repo
    let repoRoot;
    try {
