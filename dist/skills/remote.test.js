...
import { parseRemoteRef, formatRemoteRef, getSkillsCacheDir, getRemotePath, getStatePath, loadState, saveState, getCacheTtlSeconds, shouldRefresh, discoverRemoteSkills, resolveRemoteSkill, } from './remote.js';
 ...
 */
function createFileTree(basePath, files) {
...
describe('discoverRemoteSkills', () => {
    ...
    describe('marketplace format', () => {
        function marketplaceJson(plugins) {
...
describe('resolveRemoteSkill', () => {
    ...
    it('resolves skill when directory name differs from skill name', async () => {
        ...
        writeFileSync(join(remotePath, 'skills', 'react-best-practices', 'SKILL.md'), `---
...
`);
    ...
    it('throws helpful error when skill not found', async () => {
        ...
        writeFileSync(join(remotePath, 'security-review', 'SKILL.md'), `---
