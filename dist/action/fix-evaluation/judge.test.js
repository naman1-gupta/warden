...
describe.skipIf(!process.env['ANTHROPIC_API_KEY'])('evaluateFix (live)', () => {
    ...
    const baseComment = {
        ...
        description: 'User input is concatenated directly into SQL query without parameterization. Use parameterized queries instead.',
        contentHash: 'abc123',
        isWarden: true,
        threadId: 'thread-1',
    };
    function makeContext(patches) {
        return {
            octokit: {}, // Tools that need octokit won't be called since we provide patches
            owner: 'test',
            repo: 'test',
            baseSha: 'base123',
            headSha: 'head456',
...
`.trim(),
    ...
    it('returns attempted_failed when fix is incomplete', async () => {
        const comment = {
            ...
            description: 'User-provided name and email are rendered without escaping. Both fields must be escaped.',
        };
        const input = {
            comment,
            changedFiles: ['src/handler.ts'],
