...
async function executeAllTriggers(matchedTriggers, octokit, context, config, inputs) {
    ...
    return processInBatches(matchedTriggers, (trigger) => executeTrigger(trigger, {
        octokit,
        context,
        config,
 ...
 */
async function postReviewsAndTrackFailures(octokit, context, results, inputs) {
 ...
 */
async function evaluateFixesAndResolveStale(octokit, context, fetchedComments, allFindings, canResolveStale, anthropicApiKey) {
...
export async function runPRWorkflow(octokit, inputs, eventName, eventPath, repoPath) {
    const { context, config, matchedTriggers } = await initializeWorkflow(octokit, inputs, eventName, eventPath, repoPath);
    if (matchedTriggers.length === 0) {
    ...
    await evaluateFixesAndResolveStale(octokit, context, reviewPhase.fetchedComments, allFindings, canResolveStale, inputs.anthropicApiKey);
