...
describe('runScheduleWorkflow', () => {
    ...
    describe('issue and PR creation', () => {
        ...
        it('uses custom issue title from schedule config', async () => {
            ...
            expect(mockCreateOrUpdateIssue).toHaveBeenCalledWith(mockOctokit, 'test-owner', 'test-repo', expect.any(Array), expect.objectContaining({
    ...
    describe('failure and error handling', () => {
        it('fails when failOn threshold is met', async () => {
            const finding = createFinding({ severity: 'high' });
            mockRunSkill.mockResolvedValue(createSkillReport({ findings: [finding] }));
            await expect(runScheduleWorkflow(mockOctokit, createDefaultInputs({ failOn: 'high' }), SCHEDULE_FIXTURES)).rejects.toThrow('setFailed');
