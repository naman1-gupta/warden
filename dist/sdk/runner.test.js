...
import { extractFindingsJson, extractBalancedJson, extractFindingsWithLLM, truncateForLLMFallback, buildSystemPrompt, estimateTokens, isRetryableError, isAuthenticationError, isAuthenticationErrorMessage, calculateRetryDelay, aggregateUsage, WardenAuthenticationError, validateFindings, generateShortId, } from './runner.js';
import { APIError, RateLimitError, InternalServerError, APIConnectionError, APIConnectionTimeoutError, BadRequestError, AuthenticationError, } from '@anthropic-ai/sdk';
...
describe('truncateForLLMFallback', () => {
    ...
    it('includes truncation marker when findings section is truncated', () => {
        const longFindings = '{"findings": [' + '{"id": "item"},'.repeat(100) + '{"id": "last"}]}';
...
describe('buildSystemPrompt', () => {
    ...
    it('does not include resource guidance when rootDir has no resource directories', () => {
        const skill = { ...baseSkill, rootDir: tempDir };
